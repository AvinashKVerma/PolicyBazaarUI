...
app.post('/users', async (req, res) => {
  try {
    const { id, name, gender, dateOfBirth, mobile } = req.body;
    const queryText =
      'INSERT INTO insurance (id, name, gender, dob, mobile) VALUES ($1, $2, $3 ,$4, $5) RETURNING *';
    const values = [id, name, gender, dateOfBirth, mobile];
...
app.put('/update/:id', async (req, res) => {
  ...
  try {
    const queryText = `
      UPDATE insurance
      SET name = $1, gender = $2, dob = $3, mobile = $4
      WHERE id = $5
      RETURNING *`;
    const values = [name, gender, dob, mobile, id];
